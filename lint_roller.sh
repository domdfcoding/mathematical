...
if [ -z "$(git status --porcelain --untracked-files=no)" ] || [ "$1" == "-f" ]; then
  ...
  autopep8 --in-place --select "$errors" -a --recursive mathematical/
  autopep8 --in-place --select "$belligerent" -a -a -a -a -a --recursive mathematical/
  >&2 flake8 --select "$errors$belligerent" mathematical/
